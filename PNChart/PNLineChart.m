...
- (void)setYLabels {
    CGFloat yStep = (_yValueMax - _yValueMin) / _yStepNum;
    yStepHeight = _chartCavanHeight / _yStepNum;
...
- (void)setYLabels:(NSArray *)yLabels withHeight:(CGFloat)height {
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < yLabels.count; index++) {
            ...
            NSInteger y = (NSInteger) (_chartMarginTop + _chartCavanHeight - index * yStepHeight);

            PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, 0, (NSInteger) _chartMarginLeft * 0.95, (NSInteger) _yLabelHeight)];
...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width {
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = index * _xLabelWidth + _chartMarginLeft*2;
            NSInteger y = _chartCavanHeight + _chartMarginBottom + _chartMarginBottom /2.0f;
...
//            label.layer.borderColor = [UIColor colorWithRed:0.5 green:0.5 blue:1 alpha:0.7].CGColor;
            [label setTextAlignment:NSTextAlignmentCenter];
            label.text = labelText;
//            label.backgroundColor = [UIColor colorWithRed:0.5 green:0.5 blue:1 alpha:0.5];
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            int x = _chartMarginLeft + _chartMarginLeft/2 + ( i * xStepWidth );
...
- (void)setChartData:(NSArray *)data {
    if (data != _chartData) {
        ...
        xStepWidth = _chartCavanWidth / (_xLabels.count - 1); 
        yStepHeight = _chartCavanHeight /(_yLabels.count - 1);
        if (!_showLabel) {
...
//            _chartCavanWidth = self.frame.size.width;
            ...
            for ( int i=0; i<_xChartLabels.count; i++) {
                PNChartLabel *label = _xChartLabels[i];
                float xLabelHeight = _chartMarginBottom /2.0f;
                NSInteger x = _chartMarginLeft + _chartMarginLeft/2 + ( i * xStepWidth );
                NSInteger y = _chartMarginTop + _chartCavanHeight + xLabelHeight;
                label.center = CGPointMake(x,y);
                [label setTextAlignment:NSTextAlignmentCenter];
...
//                label.backgroundColor = [UIColor colorWithRed:0.5 green:0.5 blue:1 alpha:0.5];
            ...
            for (int index = 0; index < _yChartLabels.count; index++) {
                NSInteger y = (NSInteger) (_chartMarginTop + _chartCavanHeight - index * yStepHeight);
...
- (void)drawRect:(CGRect)rect {
    // !!!!! core graphics coordinate system, left top is original !!!!
    if (self.isShowCoordinateAxis) {
        CGFloat intervalWithYLabel = 5.0f;
...
//        CGFloat xAxisWidth = _chartMarginLeft + _chartCavanWidth;
//        CGFloat yAxisHeight = _chartMarginTop + _chartCavanHeight;

        // draw coordinate axis
        CGContextMoveToPoint(ctx, _chartMarginLeft + intervalWithYLabel, 0);
        CGContextAddLineToPoint(ctx, _chartMarginLeft + intervalWithYLabel, _chartMarginTop + _chartCavanHeight);
        CGContextAddLineToPoint(ctx, _chartMarginLeft + _chartCavanWidth + _chartMarginRight, _chartMarginTop + _chartCavanHeight);
        CGContextStrokePath(ctx);

        // draw y axis arrow
        CGContextMoveToPoint(ctx, _chartMarginLeft + intervalWithYLabel - 3, 6);
        CGContextAddLineToPoint(ctx, _chartMarginLeft + intervalWithYLabel, 0);
        CGContextAddLineToPoint(ctx, _chartMarginLeft + intervalWithYLabel + 3, 6);
        CGContextStrokePath(ctx);

        // draw x axis arrow
        CGContextMoveToPoint(ctx, _chartMarginLeft + _chartCavanWidth + _chartMarginRight - 6, _chartMarginTop + _chartCavanHeight - 3);
        CGContextAddLineToPoint(ctx,_chartMarginLeft + _chartCavanWidth + _chartMarginRight, _chartMarginTop + _chartCavanHeight);
        CGContextAddLineToPoint(ctx, _chartMarginLeft + _chartCavanWidth + _chartMarginRight - 6, _chartMarginTop + _chartCavanHeight + 3);
        CGContextStrokePath(ctx);
        ...
        if (self.showLabel) {
            ...
            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                float x = _chartMarginLeft + _chartMarginLeft/2 + ( i * xStepWidth );
                float y = _chartMarginTop + _chartCavanHeight;
            ...
            for (NSUInteger i = 0; i < [self.yLabels count]; i++) {
                point = CGPointMake(_chartMarginLeft + intervalWithYLabel, _chartMarginTop + _chartCavanHeight - ( i * yStepHeight ) );
    ...
    if (self.showYGridLines) {
        CGContextRef ctx = UIGraphicsGetCurrentContext();
        CGFloat intervalWithYLabel = _showLabel ? 10.f : 0.0f;
        CGPoint point;
//        CGFloat yStepHeight = _chartCavanHeight / _yStepNum;
        if (self.yGridLinesColor) {
            CGContextSetStrokeColorWithColor(ctx, self.yGridLinesColor.CGColor);
        } else {
            CGContextSetStrokeColorWithColor(ctx, [UIColor lightGrayColor].CGColor);
        }
        for (NSUInteger i = 1; i < [self.yLabels count]; i++) {
            point = CGPointMake(_chartMarginLeft + intervalWithYLabel, _chartMarginTop + _chartCavanHeight - ( i * yStepHeight ) );
            CGContextMoveToPoint(ctx, point.x, point.y);
            ...
            CGContextAddLineToPoint(ctx, _chartMarginLeft + _chartCavanWidth + _chartMarginRight, point.y);
            CGContextStrokePath(ctx);
        }
    }
